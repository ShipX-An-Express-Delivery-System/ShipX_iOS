...
struct TravellerDetailsView: View {
    ...
    func getShippingDetailsData() {
        getShippingProcessRequest(email: UserDefaults.standard.string(forKey: "email") ?? "", accessToken: UserDefaults.standard.string(forKey: "accessToken") ?? "", URLString: .shippingDetailsDataForTravellerURL(), completion: { (response, error) in
